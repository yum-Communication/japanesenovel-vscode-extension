 ...
 * @returns 空白を除去する関数
 */
function dashConv(dashNumber: string, dashType: string): NovelFormatter {
	const rgx1 = /[—―─]/g;
	const rgx2014a = /——/g;	// U+2014 EM DASH 全角ダッシュ
	const rgx2014b = /—/g;	// U+2014 EM DASH 全角ダッシュ
	const rgx2015a = /――/g;	// U+2015 ホリゾンタルバー
	const rgx2015b = /―/g;	// U+2015 ホリゾンタルバー
	const rgx2500a = /──/g;	// U+2500 罫線
	const rgx2500b = /─/g;	// U+2500 罫線
	const rgxX = /\u000b/g; // U+000b
	const b = dashNumber === '偶数個にする';
	return (row: string): string => {
		if (dashType === 'U+2015 ―水平棒') {
			row = row.replace(rgx1, '―');
			if (b) {
				row = row.replace(rgx2015a, '\u000b')
					.replace(rgx2015b, '\u000b')
					.replace(rgxX, '――');
			}
		} else if (dashType === 'U+2500 ─罫線') {
			row = row.replace(rgx1, '─');
			if (b) {
				row = row.replace(rgx2500a, '\u000b')
					.replace(rgx2500b, '\u000b')
					.replace(rgxX, '─');
			}
		} else if (b) {
			row = row.replace(rgx2014a, '\u000b')
				.replace(rgx2014a, '\u000b')
				.replace(rgxX, '——')
				.replace(rgx2015a, '\u000b')
				.replace(rgx2015b, '\u000b')
...
function dotLeaderConv(dotLeader: string): NovelFormatter {
	const rgx1 = /(\.\.\.|．．．|・・・|･･･)/g;
	const rgx2 = /……/g;
	const rgx3 = /…/g;
	const rgx4 = /\.\.\./g;
	return (row: string): string => {
		if (dotLeader !== 'そのまま') {
			row = row.replace(rgx1, '…');
			if (dotLeader === "'…'にして偶数個に揃える") {
				row = row.replace(rgx2, '...').replace(rgx3, '...').replace(rgx4, '……');
			}
		}
		return row;
	};
...
export function formatDocument(ary: string[], convertType: ConvertType, diagnostics: Diagnostic[]): string {
	...
	// ダッシュの変換
	formatter.push(dashConv(cf.dashNumber, cf.dashType));
	// 三点リーダーの変換
	formatter.push(dotLeaderConv(cf.dotLeader));
